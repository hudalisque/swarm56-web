...
범위(Peter 지시): **추가 기능만.** 삭제/편집/순서변경 UI 없음 — 필요 시 v1 문서 §9 수기 런북으로 처리.
...
**③ 결정 맥락 (Peter, 2026-07-09):** "이번엔 간단하게. 필요해지면 원칙대로." — HTML→MD 변환도 웹이 쓰는 건 동일하므로 예외 회피가 아님을 확인 후, 직접 쓰기 채택.
**업그레이드 경로(원칙 무손상 필요 시):** 웹은 중립 대기폴더(예: `/var/lib/swarm56/inbox/`)에만 쓰고, systemd `.path` 유닛이 감지해 `project/`로 이동 — "지금 클리핑" 트리거와 동일한 검증된 패턴 재사용.
